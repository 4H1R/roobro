...
describe("meeting room leave dialog", () => {
  ...
  it("keeps keyboard focus inside the confirmation and restores it on Escape", async () => {
    ...
    const leave = container.querySelector<HTMLButtonElement>('.hangup')!
    leave.focus()
    act(() => leave.click())

    const dialog = container.querySelector<HTMLDivElement>('[role="dialog"][aria-modal="true"]')!
    expect(dialog.getAttribute("aria-labelledby")).toBe("leave-dialog-title")
